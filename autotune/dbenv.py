...
class DBEnv:
    ...
    def get_workload(self):
        if self.args_db["workload"] == "sysbench":
            wl = dict(SYSBENCH_WORKLOAD)
            wl["type"] = self.args_db["workload_type"]
        elif self.args_db["workload"].startswith("oltpbench_"):
            wl = dict(OLTPBENCH_WORKLOADS)
        elif self.args_db["workload"] == "job":
            wl = dict(JOB_WORKLOAD)
        elif self.args_db["workload"] == "tpch":
    ...
    def get_external_metrics(self, filename=""):
        if self.workload["name"] == "sysbench":
            ...
            select_file = dirname + "/cli/selectedList_{}.txt".format(
                self.workload["name"]
            )
    ...
    def get_benchmark_cmd(self, oltpbench_config_xml=None):
        ...
        if self.workload["name"] == "sysbench":
            ...
            cmd = self.workload["cmd"].format(
                dirname + "/cli/run_job_{}.sh".format(self.db.args["db"]),
            ...
            cmd = self.workload["cmd"].format(
                dirname + "/cli/run_tpch_{}.sh".format(self.db.args["db"]),
