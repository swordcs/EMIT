...
class DBTuner:
    ...
    def load_history(self, knob_num):
        ...
        config_space = self.setup_configuration_space(self.args_db['knob_config_file'], knob_num)
        for f in files:
            if self.args_tune['task_id'] in f:
                continue
            try:
                task_id = f.split('.')[0]
                ...
                self.history_container_list.append(history_container)
            except:
    ...
    def setup_transfer(self):
        ...
        elif self.transfer_framework == 'what_if':
            self.surrogate_type = 'tlbo_what_if'
            if self.method != 'SMAC':
                raise ValueError('We currently only support SMAC. Invalid method for context!')
            self.load_history(-1)

        elif self.transfer_framework == 'fine_grained':
            self.surrogate_type = 'tlbo_fine_grained'
            if self.method != 'SMAC':
                raise ValueError('We currently only support SMAC. Invalid method for context!')
            self.load_history(-1)
        else:
        ...
        if len(self.history_container_list)==0 and self.space_transfer:
            self.load_history(-1)
    ...
    def tune(self):
        bo = PipleLine(self.env.step,
                       ...
                       history_bo_data=self.history_container_list,
                       acq_type=self.args_tune.get('acq_type', 'auto'),
                       ...
                       init_strategy='random_explore_first',#  latin_hypercube
                       ...
                       data_repo=self.args_tune['data_repo'],
        ...
        if history.num_objs == 1:
            ...
            plt.savefig(f'figure/{history.task_id}_{time.ctime().replace(" ","_")}.png')
            plt.close()
