...
def build_surrogate(func_str='gp', config_space=None, rng=None, history_hpo_data=None, context=None):
    ...
    elif func_str.startswith('rule_fit'):
        from autotune.optimizer.surrogate.rule_fit import RuleFit
        return RuleFit(configspace=config_space, types=types, bounds=bounds, feature_names= [e.name for e in config_space.get_hyperparameters()])
    ...
    if func_str == 'random_forest':
        ...
        if 'rgpe' in func_str:
            ...
            return WorkloadMapping(config_space, history_hpo_data, seed,
                                   surrogate_type=inner_surrogate_type, num_src_hpo_trial=-1)
        # Added by Jian Geng
        elif 'fine_grained' in func_str:
            from autotune.transfer.tlbo.fine_grained import FineGrained
            inner_surrogate_type = func_str.replace("tlbo_","")
            return FineGrained(config_space, history_hpo_data, seed,
                                   surrogate_type=inner_surrogate_type, num_src_hpo_trial=-1)
        elif 'what_if' in func_str:
            from autotune.transfer.tlbo.what_if import WhatIf
            inner_surrogate_type = 'rule_fit'
            return WhatIf(config_space, history_hpo_data, seed,
                                   surrogate_type=inner_surrogate_type, num_src_hpo_trial=-1)
        else:
