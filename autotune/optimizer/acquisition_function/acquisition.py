...
from autotune.utils.normalization import normalize_y, zero_mean_unit_var_normalization, zero_one_normalization
...
class EIT(AbstractAcquisitionFunction):
    ...
    def __init__(self,
        ...
        super(EIT, self).__init__(model)
        self.long_name = 'Expected Improvement and Transfer'
        self.par = par
        self.eta = None
    ...
    def _compute(self, X: np.ndarray, **kwargs):
        ...
        y = normalize_y(y=y, normalize=self.normalize)
        self.eta = min(y)

        m1, v1, m2, _, votes, beta = self.model.predict(X)
        s1 = np.sqrt(v1)
        ...
        def calculate_f():
            if len(X) > 1:
                z1 = (self.eta - m1 - self.par) / s1
                return (1 - beta) * (
                    (self.eta - m1 - self.par) * norm.cdf(z1) + s1 * norm.pdf(z1)
                ) + beta * ((self.eta - m2 - self.par) + votes)
            else:
                return (self.eta - m2 - self.par) + votes

        if np.any(s1 == 0.0) and len(X) > 1:
            # if std is zero, we have observed x on all instances
            # using a RF, std should be never exactly 0.0
            # Avoid zero division by setting all zeros in s to one.
            # Consider the corresponding results in f to be zero.
...
class FineGrained(AbstractAcquisitionFunction):
    def __init__(self,
                 model: AbstractModel,
                 **kwargs):

        super(WhatIf, self).__init__(model)
        self.long_name = 'fine grained'
...
class WhatIf(AbstractAcquisitionFunction):
