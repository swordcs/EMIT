...
class BO_Optimizer(object, metaclass=abc.ABCMeta):
    ...
    def check_setup(self):
        ...
        if self.num_objs == 1:
            if self.num_constraints == 0:
                assert self.acq_type in ['ei', 'eips', 'logei', 'pi', 'lcb', 'lpei', 'what_if', 'eit']
            else:  # with constraints
    ...
    def create_initial_design(self, init_strategy='default', excluded_configs=None, random_state=42):
        ...
        if init_strategy == 'random':
            ...
            lhs = LatinHypercubeSampler(self.config_space, num_random_config, criterion='maximin', random_state=random_state)
    ...
    def get_surrogate(self, history_container: HistoryContainer):
        ...
        if self.num_objs == 1:
            ...
            self.surrogate_model.train(X, self.scalarized_obj(Y))
        else:  # multi-objectives
    ...
    def get_suggestion(self, history_container: HistoryContainer, return_list=False, compact_space=None):
        ...
        # triggered if alter model is activated
        ...
        if self.optimization_strategy == 'bo':
            ...
            if self.num_objs == 1:
                ...
                if self.acq_type == 'parego':
                    self.acquisition_function.update(model=self.surrogate_model,
                                                     constraint_models=self.constraint_models,
                                                     eta=self.scalarized_obj(np.atleast_2d(mo_incumbent_value)),
                                                     num_data=num_config_evaluated,
                                                     compact_space=compact_space,
                                                     incumbent=history_container.get_incumbents()[0][0]
                                                     )
