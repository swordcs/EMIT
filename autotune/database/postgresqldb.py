...
class PostgresqlDB:
    def __init__(self, args):
        ...
        self.postgres = os.path.join(
            os.path.split(os.path.abspath(self.pg_ctl))[0], "postgres"
        )
        ...
        if self.remote_mode:
            ...
            self.ssh_pk_file = os.path.expanduser(args["private_key"])
            self.pk = paramiko.Ed25519Key.from_private_key_file(self.ssh_pk_file)
        ...
        self.PG_STAT_VIEWS = [
            "pg_stat_archiver",
            "pg_stat_bgwriter",  # global
            "pg_stat_database",
            "pg_stat_database_conflicts",  # local
            "pg_stat_user_tables",
            "pg_statio_user_tables",  # local
            "pg_stat_user_indexes",
            "pg_statio_user_indexes",  # local
        ]
        self.PG_STAT_VIEWS_LOCAL_DATABASE = [
            "pg_stat_database",
            "pg_stat_database_conflicts",
        ]
        self.PG_STAT_VIEWS_LOCAL_TABLE = [
            "pg_stat_user_tables",
            "pg_statio_user_tables",
        ]
        self.PG_STAT_VIEWS_LOCAL_INDEX = [
            "pg_stat_user_indexes",
        ...
        self.num_metrics = len(self.NUMERIC_METRICS)
        self.im_alive_init()

        # PostgreSQL Knobs
        self.knobs_detail = initialize_knobs(
            args["knob_config_file"], int(args["knob_num"])
        )
        self.default_knobs = get_default_knobs()

        self.clear_cmd = f"""psql -c \"select pg_terminate_backend(pid) from pg_stat_activity where datname = '{self.dbname}';\" """

    def _gen_config_file(self, knobs):
        if self.remote_mode:

            cnf = f"/tmp/pglocal_{int(time.time())}.cnf"
    ...
    def _kill_postgres(self):
        ...
        force_kill_cmd1 = (
            "ps aux|grep '" + self.sock + "'|awk '{print $2}'|xargs kill -9"
        )
        force_kill_cmd2 = (
            "ps aux|grep '" + self.pgcnf + "'|awk '{print $2}'|xargs kill -9"
        )
        ...
        if self.remote_mode:
            ...
            _, ssh_stdout, _ = ssh.exec_command(kill_cmd)
            ...
            try:
                _, _ = p_close.communicate(timeout=60)
    ...
    def _start_postgres(self):
        if self.remote_mode:
            ...
            start_cmd = "{} --config_file={} -D {}".format(
                self.postgres, self.pgcnf, self.pgdata
            )
            ...
            if self.isolation_mode:
                cgroup_cmd = "sudo -S cgclassify -g memory,cpuset:server " + str(
                    self.pid
                )
                ...
                if not ret_code:
                    ...
                    logger.info(
                        "Failed: add {} to memory,cpuset:server".format(self.pid)
                    )
    ...
    def apply_default_knobs_offline(self):
        self.apply_knobs_offline(self.default_knobs)
        logger.info("load default knobs finished!")
    ...
    def get_internal_metrics(
        self, internal_metrics, BENCHMARK_RUNNING_TIME, BENCHMARK_WARMING_TIME
    ):
        _counter = 0
        _period = 5
    ...
    def _post_handle(self, metrics):
        def do(metric_name, metric_values):
            return float(float(metric_values[-1]) - float(metric_values[0])) / len(
                metric_values
            )

        # logger.info("metrics before handle: " + str(metrics))
    ...
    def analyze(self):
        analyze_cmd = f"{os.path.dirname(self.pg_ctl)}/psql --username {self.user} -d {self.dbname} -c analyze"
        if self.remote_mode:
            ssh = paramiko.SSHClient()
            ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            ssh.connect(
            ...
            logger.info("Analyze ssh connect finished!")
            _, ssh_stdout, _ = ssh.exec_command(analyze_cmd, timeout=600)
            ret_code = ssh_stdout.channel.recv_exit_status()
            if ret_code != 0:
            ...
            _, _ = p_close.communicate(timeout=600)
            ret_code = p_close.poll()
            if ret_code != 0:
                logger.info("Analyze failed!")
