...
def normalize_y(y, normalize, lower=None, upper=None, mean=None, std=None):
    if normalize == 'standardize':
        if (y == y[0]).all():
            y[0] += 1e-4
        y, _, _ = zero_mean_unit_var_normalization(y, mean=mean, std=std)
    elif normalize == 'scale':
        if (y == y[0]).all():
            y[0] += 1e-4
        y, _, _ = zero_one_normalization(y, lower=lower, upper=upper)
        y = 2 * y - 1.
    return y
